...
def process_query(original_query):
    # Ask the user how they want the response
    print("\nWould you like the answer to be:")
    print("1. 💡 AI-enhanced guidance (with friendly explanation based on Prabhupada’s teachings)")
    print("2. 📜 Only original texts from Srila Prabhupada (verse, translation, purport)")
    ...
    query_refinement_response = client.chat.completions.create(
        messages=[
            ...
            "content": (
                ...
                "IMPORTANT: When the user refers to 'previously discussed' topics or content, carefully analyze ALL previous queries to identify the specific topics, concepts, or verses that were mentioned before. "
                "Instead of returning vague references to 'previous discussions', explicitly include the relevant topics, concepts, or verses from previous queries in the refined query. "
                "For example, if a previous query was about 'karma yoga' and the current query asks about 'previously discussed concepts', the refined query should explicitly mention 'karma yoga' rather than just saying 'previously discussed concepts'.\n\n"
                
                "Be comprehensive in identifying relevant context from the conversation history. "
                "Be accurate and brief in your refinement."
            )
        },
        {
            "role": "user",
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "   - Include the URL for the verse in the following format:\n"
                    "     https://github.com/iskconpress/books/blob/master/bg/[chapter]/[verse].txt\n"
                    "     (e.g., https://github.com/iskconpress/books/blob/master/bg/1/1.txt)\n"
                    "   - Do NOT use vedabase.io links. If any vedabase URL appears, replace it with the corresponding GitHub ISKCON Press URL only.\n\n"
    ...
    # Based on user choice, decide whether to proceed to Stage 3 or return facts directly
    if choice == "2":  # User wants only original texts
        final_answer = extracted_facts  # Use the extracted facts as the final answer
        print("\n📜 Retrieved Context Snippet:")
        print(retrieved_context[:300], "...\n")  # Print a snippet if long

        print("📝 Final Answer (Original Texts Only):")
        print(final_answer)
    else:
